...
public class Busqueda {
		...
		if (event.getComponent().getId().equals("AutoCmpNoProyecto")){
	        if (event.getComponent() instanceof SelectInputText) {
	             ...
	             if(newNoProyecto.length() > 2){
	            	 ...
	            	 int i = 0;
	            	 for(Proyecto p : (List<Proyecto>)catalogo.getProyectoDAO().findByCoincidenceQuery(queryBuilder()))
	            		 noProyectoListMatch.add(new SelectItem(i, p.getNumProy()));	   
	            	 i++;
	            }             
	...
	public String toDetalle(){	
		...
		captacion.getCarp().getFileList().clear();

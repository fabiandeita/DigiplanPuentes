...
public class Volcar {
		...
		try{
			PrintWriter salida = null;
			 ...
			 for(Documento doc : (List<Documento>)docDAO.findAllActive()){
				File plano = new File(conf.getBasePath() + conf.getResourcePath() + doc.getArchivo());				
				if(!plano.exists()){
					salida.println(doc.getIdDocumento() + " -- " + doc.getArchivo());
	...
	public void read() {
		try {
			...
			for (int i = 1; i < filas; i++) {
				...
				doc.setProyecto(getProyecto(numeroProyecto,cell.getContents()));
	...
	public Proyecto getProyecto(String numero, String nombreProyecto) {
		Proyecto p = proyectoDAO.findByNumeroProyecto(numero);		
		
		if (p != null){			
			if(!p.getNombre().equals(nombreProyecto.toUpperCase())){
				
				writeLog.println("Nombre del proyecto ya guardado:" + p.getNombre());
				writeLog.println("Nombre del proyecto diferente  :" + nombreProyecto);
				writeLog.println(" ");
			}
			return p;
		} else {

...
public class EstadoDAO extends BaseHibernateDAO {
	...
	public void save(Estado transientInstance) {
	...
	public Estado findById(java.lang.Integer id) {
		...
		try {
			Estado instance = (Estado) getSession().get("dao.Estado", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Estado instance) {
		...
		try {
			List results = getSession().createCriteria("dao.Estado")
	...
	public Estado merge(Estado detachedInstance) {
		...
		try {
			Estado result = (Estado) getSession().merge(detachedInstance);
